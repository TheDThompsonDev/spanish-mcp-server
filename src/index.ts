...
import { vocabularyData, VocabularyWord } from './data/vocabulary.js';
import { verbConjugations, VerbConjugation } from './data/conjugations.js';
import { grammarExplanations, GrammarExplanation } from './data/grammar.js';
import { culturalContext, CulturalContext } from './data/cultural.js';
...
server.setRequestHandler(ReadResourceRequestSchema, async (request: any) => {
  ...
  if (uri.startsWith('spanish://vocabulary/')) {
    ...
    const words = vocabularyData.filter((word: VocabularyWord) => word.topic === topic);
    return {
      contents: [
        {
          uri,
  ...
  if (uri.startsWith('spanish://mixed/')) {
    ...
    const levelWords = vocabularyData.filter((word: VocabularyWord) => word.difficulty === level);
    const relevantGrammar = Object.entries(grammarExplanations)
      .filter(([_, grammar]: [string, GrammarExplanation]) => grammar.difficulty === level)
...
server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
  ...
  switch (name) {
    case 'translate_word': {
      const { word, direction } = args as { word: string; direction: string };
      ...
      const vocabWord = vocabularyData.find((v: VocabularyWord) =>
        v[searchField].toLowerCase() === word.toLowerCase()
      );
      
      const result: TranslationResult = vocabWord ? {
    ...
    case 'conjugate_verb': {
      const { verb, tense = 'present' } = args as { verb: string; tense?: string };
      
      if (!verb || typeof verb !== 'string') {
        throw new McpError(ErrorCode.InvalidRequest, 'Verb parameter is required and must be a string');
      }
      
      const validTenses: (keyof VerbConjugation)[] = ['present', 'preterite', 'imperfect', 'future', 'conditional'];
      if (!validTenses.includes(tense as keyof VerbConjugation)) {
        throw new McpError(ErrorCode.InvalidRequest, `Tense must be one of: ${validTenses.join(', ')}`);
      }
      
      const conjugation: VerbConjugation | undefined = verbConjugations[verb.toLowerCase()];
      if (!conjugation) {
        return {
          content: [
            {
      ...
      const tenseConjugation = conjugation[tense as keyof VerbConjugation];
      if (!tenseConjugation) {
        return {
          content: [
            {
    ...
    case 'get_vocabulary_by_topic': {
      const { topic, difficulty, limit = 10 } = args as {
        topic: string;
        difficulty?: string;
        limit?: number;
      };
      ...
      let words = vocabularyData.filter((word: VocabularyWord) => word.topic === topic);
      
      if (difficulty) {
        words = words.filter((word: VocabularyWord) => word.difficulty === difficulty);
      }
    ...
    case 'get_cultural_context': {
      const { word_or_phrase } = args as { word_or_phrase: string };
      ...
      const context: CulturalContext | undefined = culturalContext[word_or_phrase.toLowerCase()];
      if (!context) {
        return {
          content: [
            {
    ...
    case 'track_progress': {
      const { user_id, activity, score } = args as {
        user_id: string;
        activity: string;
        score?: number;
      };
